...
neutralResultsList<-list()
for (sim in 1:100){
  tempFile<-read.csv(paste0("/home/ch19g17/Sim_for_variable_recombination_zalpha/neutral/sim_",sim,"/simResults.csv"))
  neutralResultsList[[paste0("sim_",sim)]]<-list()
  for (i in 1:36){
    neutralResultsList[[paste0("sim_",sim)]][[colnames(tempFile)[i]]]<-tempFile[,i]
  }
  tempFile<-read.csv(paste0("/home/ch19g17/Sim_for_variable_recombination_zalpha/selected/sim_",sim,"/simResults.csv"))
  selectedResultsList[[paste0("sim_",sim)]]<-list()
  for (i in 1:36){
    selectedResultsList[[paste0("sim_",sim)]][[colnames(tempFile)[i]]]<-tempFile[,i]
  }
  tempFile<-read.csv(paste0("/home/ch19g17/Sim_for_variable_recombination_zalpha/selected/sim_",sim,"/simMidResults.csv"))
...
setwd("/home/ch19g17/Sim_for_variable_recombination_zalpha/Graphs/new")
