...
MAF<-read.table("H:/Dogs/cluster2/Candidate Regions/MAFs.txt",header = TRUE)
MAF<-MAF[,c(2,7,3,4,5)]
candSNPs<-merge(candSNPs,MAF)
candSNPs<-candSNPs[order(candSNPs$SNP),c(3,1,2,4:12)]

write.table(candSNPs[,c(-1)],"H:/Dogs/cluster2/Candidate Regions/CandSNPs.txt",row.names = FALSE,col.names = TRUE,quote = FALSE)
...
jpeg("\\\\filestore.soton.ac.uk/users/ch19g17/mydocuments/Dogs/cluster2/Candidate Regions/Zalpha.jpeg",res=300,height=10,width=20,units='cm',quality=100)
...
jpeg("\\\\filestore.soton.ac.uk/users/ch19g17/mydocuments/Dogs/cluster2/Candidate Regions/Zroe_Zb_manhat.jpeg",res=300,height=18,width=25,units='cm',quality=100)
...
write.table(finalCandSNPs[,c(-1)],"H:/Dogs/cluster2/Candidate Regions/FinalCandSNPs.txt",row.names = FALSE,col.names = TRUE,quote = FALSE)
...
write.csv(annogene,"\\\\filestore.soton.ac.uk/users/ch19g17/mydocuments/Dogs/cluster2/Candidate Regions/ChIPoutput.csv",quote = FALSE)
